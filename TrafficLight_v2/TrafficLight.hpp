...
namespace crt
{
	...
	class TrafficLight : public Task{
		...
		private:
			...
			Flag flagKnopIngedruktA;
			Flag flagKnopIngedruktB;
			State state = State::GreenLed;

		public:

			TrafficLight (const char *taskName, unsigned int taskPriority, unsigned int taskCoreNumber, const uint8_t pinRled, uint8_t pinGled)
				: Task(taskName, taskPriority, 3000, taskCoreNumber), greenLed(static_cast<gpio_num_t>(pinGled), LEDC_CHANNEL_0, LEDC_TIMER_0),
				redLed(static_cast<gpio_num_t>(pinRled), LEDC_CHANNEL_1, LEDC_TIMER_0), PIN_RLED(pinRled), PIN_GLED(pinGled), flagKnopIngedruktA(this), flagKnopIngedruktB(this)
			...
			void knopIngedruktA()
    		{
        	flagKnopIngedruktA.set();
   			}

			void knopIngedruktB()
    		{
        	flagKnopIngedruktB.set();
   			}
				...
				while (true)
				{
					switch(state){
						case State::GreenLed:{
							...
							redLed.setDuty(0); 
							vTaskDelay(100);
							wait(flagKnopIngedruktA);
							if (flagKnopIngedruktA) {
						...
						case State::OrangeLed:{
							...
							blinks = 0;
							vTaskDelay(100);
							wait(flagKnopIngedruktB);
							if (flagKnopIngedruktB) {
