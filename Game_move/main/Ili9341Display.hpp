 ...
 * 
 * Implements an entity that can be moved. Test the moving of the entity it in the diagnostics_moving method. 
 */
#pragma once
...
public:
    ...
    // Two screens differ in their orrientation for CYD3.2 in landscape it is: 320(w)x240(h). You can probably flip orientation in you config lcd setup (I did not investigate).
    ...
    void drawSquare(int x, int y, int w, uint16_t color, bool fill = false);
    ...
    void plotLineHigh(int x0, int y0, int x1, int y1, uint16_t color);
    void plotLineLow(int x0, int y0, int x1, int y1, uint16_t color);
