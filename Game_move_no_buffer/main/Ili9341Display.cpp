...
// verander de fillScreen functie zodat het werkt zonder buffer, maar met iets anders
...
void Ili9341Display::plotLineLow(int x0, int y0, int x1, int y1, uint16_t color) {
    int dx = x1 - x0;
    int dy = y1 - y0;
    int yi = 1;
    if (dy < 0)
...
void Ili9341Display::plotLineHigh(int x0, int y0, int x1, int y1, uint16_t color) {
    int dx = x1 - x0;
    int dy = y1 - y0;
    int xi = 1;
    if (dx < 0)
...
void Ili9341Display::drawRectangle(int x, int y, int w, int h, uint16_t color, bool fill) {
    ...
    if (fill)
    {
        for (int startX = x; startX < (x + w); startX++){
            for (int startY = y; startY < (y + h); startY++){
...
void Ili9341Display::drawSquare(int x, int y, int w, uint16_t color, bool fill) {
    // from top left corner
    drawLine(x, y, x + w, y, color);
    drawLine(x, y, x, y + w, color);

    // from opposite corner
    drawLine(x + w, y + w, x + w, y, color);
